...
1. パッケージ実体の LICENSE ファイル（レジストリの tarball から取得）
2. パッケージメタデータの SPDX 式
3. `license` / `licenses` フィールド（自己申告。最も信用度が低い）
4. ClearlyDefined のキュレーション済みデータ
5. ユーザーによる手動オーバーライド
...
| 事象 | 挙動 |
|---|---|
| レジストリ到達不可 | `unresolved` として記録。ブロックしない。指数バックオフでリトライ |
| 未知のライセンス文字列 | `review`（`blocked` にしない） |
| ライセンス表記が完全に不在 | `blocked`（全権利留保のため） |

...
<textarea id="content" placeholder="Paste a lockfile (package-lock.json, pnpm-lock.yaml, yarn.lock, go.sum, Cargo.lock, poetry.lock, Gemfile.lock, packages.lock.json), an SBOM (CycloneDX or SPDX, JSON), or a manifest (package.json, requirements.txt, pyproject.toml, go.mod, Cargo.toml, .csproj)"></textarea>
...
`;
  ...
  return renderLayout({
    ...
    description:
      'Paste package.json, requirements.txt, pyproject.toml, or go.mod and see which dependency licenses create obligations for your specific shipping model — hosted SaaS, distributed binary, customer delivery, internal use, or published library.',
    path: '/',
    body,
    script: SCRIPT,
